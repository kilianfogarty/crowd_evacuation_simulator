...
class TestIntegration:
    ...
    def test_result_stored_accurately(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            env: Environment = EnvironmentFactory.build_environment(width=20, 
                                                       height=20, 
                                                       num_agents=10, 
                                                       num_obstacles=0, 
                                                       seed=0)
    ...
    def test_fixed_seed_deterministic(self) -> None:
        ...
        def run_once() -> float | None:
            env = EnvironmentFactory.build_environment(width=20, height=20, num_agents=5, num_obstacles=0, seed=7)
        ...
        assert np.allclose(simulation_result_a, simulation_result_b) and np.allclose(simulation_result_b, simulation_result_c) and np.allclose(simulation_result_a, simulation_result_c)

import numpy as np
import pytest
from crowd_evacuation_simulator.environment_factory import Environment, EnvironmentFactory

class TestEnvironmentFactory:

    # init
    def test_environment_factory_initialization(self) -> None:
        env: Environment = EnvironmentFactory.build_environment(width=20, height=20, num_agents=10, num_obstacles=3, seed=0)
        assert env.width == 20
    ...
    def test_environment_factory_agents_placed_within_margins(self) -> None:
        env = EnvironmentFactory.build_environment(width=20, height=20, num_agents=30, num_obstacles=0, seed=0)
        margin = 0.1
    ...
    def test_obstacles_placed_within_margins(self) -> None:
        env = EnvironmentFactory.build_environment(width=20, height=20, num_agents=0, num_obstacles=10, seed=0)
        margin = 0.1
        x_min, y_min = 20 * margin, 20 * margin
        x_max, y_max = 20 * (1 - margin), 20 * (1 - margin)
    ...
    def test_same_seed_produces_identical_positions(self) -> None:
        env_a = EnvironmentFactory.build_environment(width=20, height=20, num_agents=10, num_obstacles=0, seed=42)
        env_b = EnvironmentFactory.build_environment(width=20, height=20, num_agents=10, num_obstacles=0, seed=42)
    ...
    def test_different_seeds_produce_different_positions(self) -> None:
        env_a = EnvironmentFactory.build_environment(width=20, height=20, num_agents=10, num_obstacles=0, seed=0)
        env_b = EnvironmentFactory.build_environment(width=20, height=20, num_agents=10, num_obstacles=0, seed=1)
        positions_match = all(
